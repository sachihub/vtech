...
df[, pct_4 := cut(maxs_mean_st, breaks = quantile(maxs_mean_st, probs = seq(0, 1, 0.25), na.rm = TRUE), labels = FALSE, include.lowest = TRUE)]
df[, pct_2 := pct_4]
df[pct_2 >= 2, pct_2 := 2]
...
outcome_list <- c(
  ...
  "dths_black_bpop", "dths_white_wpop", "dths_0000_pop", "dths_0144_pop",
  "dths_4564_pop", "dths_6599_pop", "dths_black_pop", "dths_white_pop"
)
...
run_model <- function(df, outcome, model) {
  fe_vars <- c("modate")  # absorbed fixed effect
  ...
  # Add extra terms depending on model
  X_extra <- character(0)
  ...
  # Adaptation models with interaction
  formula_rhs <- paste(c(X_base, X_extra), collapse = " + ")
...
for (Y in outcome_list) {
  for (m in model_list) {
    ...
    # Save for future use (RDS preferred over .dta for full model object)
...
#5. Handling Betas for Attribution (Optional)
