...
for (Y in outcome_list) {
  ...
  # Summary statistics using reframe
  mort_summary <- merged %>%
    reframe(across(all_of(mortmodel), \(x) c(sum = sum(x, na.rm = TRUE), mean = mean(x, na.rm = TRUE))))
  ...
  state_summary <- full_data_state %>%
    reframe(across(starts_with("mort_"), \(x) sum(x, na.rm = TRUE)))
